...
//
// Ported from HypercubeCNN's HCNNSpatialAug.h/.cpp for the LCN
// examples; the HCNN name prefix is dropped. Not part of the Core
// library.
 ...
 * patterns onto the network input separately (see SpatialEmbed).
 *
 ...
 * on for MNIST (see `examples/mnist/lcn_mnist.cpp`).
 *
 * Keep **aug-then-embed**; do not aug on packed vertices.
 */
struct SpatialAugConfig {
...
public:
    ...
    explicit SpatialAugmenter(SpatialAugConfig cfg = {});

    /// Replaces config after validate() (throws if invalid).
    void set_config(const SpatialAugConfig& cfg);
    const SpatialAugConfig& config() const { return cfg_; }

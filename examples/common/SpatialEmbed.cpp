...
int SpatialEmbedConfig::capacity() const {
    if (dim < 1 || dim > 30) {
        throw std::runtime_error("SpatialEmbedConfig: dim must be in [1, 30]");
    }
    return 1 << dim;
}

void SpatialEmbedConfig::validate() const {
    if (dim < 1 || dim > 30) {
        throw std::runtime_error("SpatialEmbedConfig: dim must be in [1, 30]");
    }
    const int N = 1 << dim;
    if (plane_side < 0) {
        throw std::runtime_error("SpatialEmbedConfig: plane_side must be >= 0");
    }
    if (plane_side > 0) {
        const long long S = plane_side;
        if (mode == SpatialEmbedMode::ResizeToFit) {
            if (S * S > static_cast<long long>(N)) {
                throw std::runtime_error(
                    "SpatialEmbedConfig: plane_side*plane_side exceeds N=2^dim");
            }
        } else if (mode == SpatialEmbedMode::DualPlaneResize) {
            if (2 * S * S > static_cast<long long>(N)) {
                throw std::runtime_error(
                    "SpatialEmbedConfig: 2*plane_side*plane_side exceeds N=2^dim");
            }
        }
...
int SpatialEmbedder::max_square_side(int N) {
    if (N < 1) return 0;
...
int SpatialEmbedder::max_dual_plane_side(int N) {
    if (N < 2) return 0;
...
SpatialEmbedder::SpatialEmbedder(SpatialEmbedConfig cfg)
    : cfg_(cfg) {
    cfg_.validate();
}

void SpatialEmbedder::set_config(const SpatialEmbedConfig& cfg) {
    cfg.validate();
    cfg_ = cfg;
}

int SpatialEmbedder::capacity() const {
    return cfg_.capacity();
}

int SpatialEmbedder::resolve_plane_side(int N) const {
    if (cfg_.plane_side > 0)
        return cfg_.plane_side;
    if (cfg_.mode == SpatialEmbedMode::DualPlaneResize)
        return max_dual_plane_side(N);
    if (cfg_.mode == SpatialEmbedMode::ResizeToFit)
        return max_square_side(N);
    return 0;
}

SpatialEmbedPlan SpatialEmbedder::plan(int height, int width) const {
    cfg_.validate();
    if (height < 1 || width < 1) {
        throw std::runtime_error(
            "SpatialEmbedder::plan: height and width must be >= 1");
    }
    ...
    case SpatialEmbedMode::PadLowCenter: {
        const long long need =
            static_cast<long long>(height) * static_cast<long long>(width);
        if (need > p.N) {
            throw std::runtime_error(
    ...
    case SpatialEmbedMode::ResizeToFit: {
        p.plane_side = resolve_plane_side(p.N);
        if (p.plane_side < 1) {
            throw std::runtime_error(
                "SpatialEmbedder::plan: ResizeToFit needs N >= 1");
        }
        p.pattern_length = p.plane_side * p.plane_side;
        break;
    }
    case SpatialEmbedMode::DualPlaneResize: {
        p.plane_side = resolve_plane_side(p.N);
        if (p.plane_side < 1) {
            throw std::runtime_error(
                "SpatialEmbedder::plan: DualPlaneResize needs N >= 2");
        }
        const long long two_planes =
            2LL * static_cast<long long>(p.plane_side) * p.plane_side;
        if (two_planes > p.N) {
            throw std::runtime_error(
                "SpatialEmbedder::plan: dual plane layout exceeds N");
        }
    ...
    default:
        throw std::runtime_error(
            "SpatialEmbedder::plan: unknown SpatialEmbedMode");
    }
    return p;
}

void SpatialEmbedder::embed(const float* in, int height, int width,
                                float* out) const {
    if (!in || !out) {
        throw std::runtime_error("SpatialEmbedder::embed: null buffer");
    }

    const SpatialEmbedPlan p = plan(height, width);
    const int N = p.N;
    ...
    case SpatialEmbedMode::DualPlaneResize: {
        ...
        if (static_cast<long long>(plane) * 2 > N) {
            throw std::runtime_error(
                "SpatialEmbedder::embed: dual plane overflow (internal)");
        }
        resize_to_square(in, height, width, out, S, pad);
    ...
    default:
        throw std::runtime_error(
            "SpatialEmbedder::embed: unknown SpatialEmbedMode");
    }
}

void SpatialEmbedder::embed_batch(const float* in, int batch,
                                      int height, int width,
                                      float* out) const {
    if (batch < 0) {
        throw std::runtime_error(
            "SpatialEmbedder::embed_batch: batch must be >= 0");
    }
    if (batch == 0)
        return;  // no-op; null buffers allowed
    if (!in || !out) {
        throw std::runtime_error("SpatialEmbedder::embed_batch: null buffer");
    }

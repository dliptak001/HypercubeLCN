...
bool SpatialAugConfig::is_identity() const {
...
void SpatialAugConfig::validate() const {
    auto require_finite = [](float v, const char* name) {
        if (!std::isfinite(v)) {
            throw std::runtime_error(
                std::string("SpatialAugConfig: ") + name + " must be finite");
        }
    };
    ...
    if (!(value_min <= value_max)) {
        throw std::runtime_error(
            "SpatialAugConfig: value_min must be <= value_max");
    }
    if (noise_sigma < 0.0f) {
        throw std::runtime_error(
            "SpatialAugConfig: noise_sigma must be >= 0");
    }
    if (elastic_alpha < 0.0f) {
        throw std::runtime_error(
            "SpatialAugConfig: elastic_alpha must be >= 0");
    }
    if (elastic_alpha > 0.0f) {
        if (elastic_sigma < kElasticSigmaMin
            || elastic_sigma > kElasticSigmaMax) {
            throw std::runtime_error(
                "SpatialAugConfig: elastic_sigma must be in "
    ...
    if (sxm * sym >= 0.95f) {
        throw std::runtime_error(
            "SpatialAugConfig: |shear_x_max|*|shear_y_max| must be < 0.95 "
            "(shear matrix near-singular)");
    }
    // rot / shift / shear maxima: magnitude is used; negative is abs.
}

SpatialAugConfig SpatialAugConfig::None() {
    SpatialAugConfig c;
    c.enabled = false;
    return c;
}
...
SpatialAugmenter::SpatialAugmenter(SpatialAugConfig cfg)
    : cfg_(cfg) {
    cfg_.validate();
}

void SpatialAugmenter::set_config(const SpatialAugConfig& cfg) {
    cfg.validate();
    cfg_ = cfg;
}

void SpatialAugmenter::apply(const float* in, float* out,
                                 int height, int width,
                                 std::mt19937& rng) const {
    if (!in || !out) {
        throw std::runtime_error("SpatialAugmenter::apply: null buffer");
    }
    if (height < 1 || width < 1) {
        throw std::runtime_error(
            "SpatialAugmenter::apply: height and width must be >= 1");
    }
    ...
    if (do_geom && in == out) {
        throw std::runtime_error(
            "SpatialAugmenter::apply: geometric aug requires in != out");
    }
...
void SpatialAugmenter::apply_batch(const float* in, float* out,
                                       int batch, int height, int width,
                                       std::mt19937& rng) const {
    if (batch < 0) {
        throw std::runtime_error(
            "SpatialAugmenter::apply_batch: batch must be >= 0");
    }
    if (!in || !out) {
        throw std::runtime_error("SpatialAugmenter::apply_batch: null buffer");
    }

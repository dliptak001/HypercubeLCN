...
EXTRACT = Path(r"C:\HypercubeLCN\RamanModels\extracted_narrow")
OUT = Path(__file__).with_name("extracted_baselines_narrowIO.png")
N = 2048


def load_row(path: Path) -> np.ndarray:
    if not path.is_file():
        raise SystemExit(f"missing {path} (run lcn_raman_extract first)")
...
def main() -> None:
    ...
    if not manifest.is_file():
        raise SystemExit(f"missing {manifest} (run lcn_raman_extract first)")

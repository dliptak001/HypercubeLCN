...
int main()
{
    ...
    try
    {
        ...
        const CoreConfig core_cfg = MakeNarrowCoreConfig();
        const TrainingConfig train_cfg = MakeTrainConfig();

        BaselineExtractor ex(core_cfg, train_cfg);
        if (ex.N() != kNarrowN)
            throw std::logic_error("extractor N must equal kNarrowN");
        s_ex = &ex;
        ...
        std::printf("io: width=%zu of N=%zu (addresses 0 .. %zu; "
                    "%zu free hidden vertices)\n",
                    kN, kNarrowN, kN - 1, kNarrowN - kN);
